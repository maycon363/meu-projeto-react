...
const Populares = () => {
  ...
  useEffect(() => {
    const buscarFilmes = async () => {
      try {
        const res = await apiFilmes.get('/movie/popular', {
          params: { language: 'pt-BR' }
        });
        setFilmes(res.data.results);
      } catch (err) {
        console.error(err);
        setErro('Erro ao carregar os filmes populares.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <Container className="letra mt-4 fade-in">
      <h1 className="text-white text-center mb-4">Filmes Populares</h1>
      <Row>
        {filmes.map((filme) => (
          <Col key={filme.id} xs={12} sm={6} md={4} lg={3} className="mb-4">
            ...
            >
              ...
              <Card.Body>
                ...
                <Card.Text>
                  <strong>Popularidade:</strong> {filme.popularity}
                </Card.Text>
              </Card.Body>
            </Card>
          </Col>
